...
import (
	...
	"github.com/gjermundgaraba/slothchain/x/wasm/ioutils"
...
// list of account types that are accepted for wasm contracts. Chains importing slothd
...
func (k Keeper) instantiate(
	...
	if existingAcct != nil {
		...
		if _, accept := k.acceptedAccountTypes[reflect.TypeOf(existingAcct)]; accept {
			...
			// consider an account in the slothd namespace spam and overwrite it.
